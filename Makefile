...
build: package
	...
	cp target/$(PROJECT_NAME)-assembly-$(VERSION).jar build/$(PROJECT_NAME)
	cp interpreter-setting.json build/$(PROJECT_NAME)
...
clean:
	...
	$(SET_SCALA_VERSION)
	$(SET_PROJECT_NAME_LOW_CASE)
	rm -rf $(PROJECT_NAME_LOW_CASE)-*.tar.gz build publish target assembly
